...
def numerical_gradient(theta, samples, epsilon=1e-6):
    ...
    base_U = lch.logchol2chol(theta)
    base_J = lch.compute_J(base_U)
    base_pi = lch.pseudo2pi(base_J)
    ...
    for i in range(len(theta)):
        ...
        U_eps = lch.logchol2chol(theta_eps)
        J_eps = lch.compute_J(U_eps)
        pi_eps = lch.pseudo2pi(J_eps)
...
def estimate_inertial_parameters(samples, learning_rate, max_iter,numerical=True):
    theta = np.random.normal(0, 0.1, size=10)

    for it in range(max_iter):
        U = lch.logchol2chol(theta)
        J = lch.compute_J(U)
        pi = lch.pseudo2pi(J)
